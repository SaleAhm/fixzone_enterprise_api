...
assert_contains "$BACKUP_TIMER" "Unit=fixzone-recovery-backup.service"
assert_contains "$BACKUP_TIMER" "OnCalendar=*-*-* 02:00:00 Africa/Lagos"
assert_contains "$BACKUP_TIMER" "Persistent=true"
assert_contains "$BACKUP_TIMER" "AccuracySec=1min"
assert_contains "$BACKUP_TIMER" "WantedBy=timers.target"
...
assert_not_contains_regex "$BACKUP_TIMER" '^Documentation=docs/'
...
assert_not_contains_regex "$BACKUP_TIMER" '(^|[^A-Za-z0-9])c3e37fb([^A-Za-z0-9]|$)|(^|[^A-Za-z0-9])a38b2a5([^A-Za-z0-9]|$)|(^|[^A-Za-z0-9])ba7e816([^A-Za-z0-9]|$)'
assert_not_contains_regex "$BACKUP_TIMER" 'FIXZONE_APPROVED_DAILY_BACKUP_TIME_REQUIRED'
assert_not_contains_regex "$BACKUP_TIMER" '^RandomizedDelaySec='
assert_not_contains_regex "$BACKUP_TIMER" '^OnUnitActiveSec='
...
assert_not_contains_regex "$BACKUP_TIMER" 'DROP DATABASE|DELETE FROM|TRUNCATE|docker[[:space:]]+(stop|restart|rm|service[[:space:]]+update)|systemctl[[:space:]]+(enable|start|restart)|rm[[:space:]]+-rf|tar[[:space:]]+-x|prisma[[:space:]]+migrate|pg_restore|pg_dump'
...
assert_not_contains_regex "$BACKUP_TIMER" 'DATABASE_URL|PASSWORD|TOKEN|SECRET|PRIVATE_KEY|SMTP|WEBHOOK|AUTHORIZATION|COOKIE'
...
assert_not_contains_regex "$BACKUP_TIMER" 'systemctl[[:space:]]+(enable|start)|systemctl[[:space:]]+enable|systemctl[[:space:]]+start'
...
assert_contains "$POLICY_DOC" "Approved daily backup time: 02:00 WAT."
assert_contains "$POLICY_DOC" "Approved timezone: Africa/Lagos."
assert_contains "$POLICY_DOC" "Materialized timer: \`ops/systemd/fixzone-recovery-backup.timer\`."
assert_contains "$POLICY_DOC" "OnCalendar contract: \`*-*-* 02:00:00 Africa/Lagos\`."
assert_contains "$POLICY_DOC" "Production timer enablement: NOT YET APPROVED/EXECUTED."
assert_contains "$RUNBOOK_DOC" "Approved daily backup time: 02:00 WAT."
assert_contains "$RUNBOOK_DOC" "OnCalendar contract: \`*-*-* 02:00:00 Africa/Lagos\`."
assert_contains "$RUNBOOK_DOC" "Production installation: NOT YET DONE."
