...
- Daily coordinated recovery backup systemd package is local review only: `ops/systemd/fixzone-recovery-backup.service` is local review ready and `ops/systemd/fixzone-recovery-backup.timer.example` is template only / not installable until schedule approval. Exact daily backup time remains an operator approval gate through `OnCalendar=FIXZONE_APPROVED_DAILY_BACKUP_TIME_REQUIRED`; production backup scheduling is not active.
