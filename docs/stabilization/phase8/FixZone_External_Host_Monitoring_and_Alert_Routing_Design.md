...
- Production diagnosis after durable verification deployment: a38b2a5 code is deployed at `/srv/securezone-ops/fixzone/current -> /srv/securezone-ops/fixzone/a38b2a5` and durable verification consumption is PASS, but state reported monitorVersion `c3e37fb` due to stale systemd environment metadata. This is a version-metadata configuration defect only, not old code execution or durable verification failure.
