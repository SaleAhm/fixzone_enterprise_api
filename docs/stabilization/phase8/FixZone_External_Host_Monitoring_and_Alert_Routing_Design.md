...
- Checksum verification visibility is no longer required to be `UNKNOWN` when the newest valid recovery set publishes trustworthy durable metadata. The monitor reads `verification-status.json` from the newest `fixzone-v1-baseline-*` or `fixzone-v1-backup-*` recovery set and treats matching `schemaVersion: 1`, matching `recoverySetId`, `state: SUCCESS`, and `checksumAlgorithm: sha256` as healthy durable verification evidence.
...
- First supervised production backup attempt failed safely at `pg_dump` role selection/authentication after capacity preflight, service resolution, and PostgreSQL major checks passed. Failed evidence remains preserved at `/srv/securezone-backups/manual/.fixzone-v1-backup-2026-08-23_12-10-17.failed`.
- PostgreSQL role hardening was implemented after the safe failure. The production role was safely verified as `postgres`; no password or connection secret is recorded in this repository.
- Second supervised production backup attempt succeeded with exit code `0`, publishing recovery set `/srv/securezone-backups/manual/fixzone-v1-backup-2026-08-23_13-35-46`.
- Post-backup production integrity remained stable: uploads `28` before and `28` after, protected baseline present, API healthy, and host monitor enabled/active.
- Independent verification classified the successful recovery set as `ARTIFACT-INTEGRITY VERIFIED`: required artifacts were present and non-empty, checksum verification passed, `pg_restore --list` structural readability passed, uploads gzip integrity passed, archive count was `28`, upload-list count was `28`, current production upload count was `28`, archive/list agreement passed, archive and upload-list canary counts were `0`, successful-run partial residue was absent, failed evidence remained preserved, baseline remained preserved, host/container uploads were `28/28`, API was healthy, and monitor was enabled/active.
- No restore was performed for the second supervised backup; the classification is not `RESTORE-PROVEN`.
- Checksum verification remains a backup-job responsibility after each successful backup. The 15-minute host monitor consumes durable verification metadata and does not re-hash recovery artifacts every cycle.
- Version metadata now follows the versioned monitor installation lifecycle. The permanent service unit must not hard-code a commit through `Environment=FIXZONE_MONITOR_VERSION=...`; the monitor uses a safe explicit override when present, otherwise derives the safe version directory basename from its resolved script path, otherwise falls back to `local`.
- Production monitor milestone after version metadata hardening: ba7e816 code is deployed at `/srv/securezone-ops/fixzone/current -> /srv/securezone-ops/fixzone/ba7e816`; manual service verification PASS; natural 15-minute timer verification PASS; stale systemd monitor-version metadata is resolved; durable verification consumption remains PASS for `fixzone-v1-backup-2026-08-23_13-35-46`.
- Daily coordinated recovery backup systemd package is local review only: `ops/systemd/fixzone-recovery-backup.service` is local review ready and `ops/systemd/fixzone-recovery-backup.timer` is locally materialized / review ready with approved `OnCalendar=*-*-* 02:00:00 Africa/Lagos` for 02:00 WAT. Production installation is not performed, production timer enablement is not performed, the first scheduled run has not occurred, and retention deletion is not approved.
